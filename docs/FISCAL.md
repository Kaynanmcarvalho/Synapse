...
As notas emitidas, a numeração, a idempotência e a fila de contingência da NFC-e também ficam no Firestore — nada fiscal fica em memória da API, então reiniciar não repete número nem perde nota:

| Dado                             | Caminho                                                   |
| -------------------------------- | --------------------------------------------------------- |
| Notas (NF-e, NFC-e, MDF-e)       | `tenants/{tenantId}/fiscalDocuments/{id}`                 |
| Último número por modelo e série | `tenants/{companyId}/fiscalSequences/{NFE\|NFCE}-{serie}` |
| Chave de idempotência → nota     | `tenants/{tenantId}/fiscalIdempotency/{sha256(chave)}`    |
| Fila de contingência da NFC-e    | `tenants/{tenantId}/nfceContingency/{documentId}`         |

A chave de idempotência, a reserva do número e o rascunho da nota são gravados numa única transação do Firestore (`FiscalRepository.reserveDocument`). Duas instâncias emitindo ao mesmo tempo nunca levam o mesmo número, e a mesma chave repetida devolve a nota que já existe sem gastar número. O teste `fiscal-numeracao.emulador.spec.ts` prova isso no emulador (`pnpm test:emulador`). O frontend nunca recebe autoridade para escolher a numeração.

O primeiro número de cada série vem do "próximo número" do Assistente de Configuração. Depois disso a sequência só avança: um "próximo número" maior pula a numeração para ele (migração de outro sistema), um menor é ignorado. Antes da primeira emissão num tenant que já emitiu por outro sistema, ajuste o "próximo número" para depois do último número usado na SEFAZ.

A empresa emitente é o próprio tenant. Emissão, inutilização, MDF-e, consulta e manifestação de DF-e recusam com 403 um `companyId` diferente do tenant da sessão, e as rotas por `:id` só encontram notas do tenant da sessão.

As migrations `apps/api/migrations/001_fiscal.sql` e `002_inbound_dfe.sql` descrevem o modelo relacional previsto em `docs/DATABASE.md`, mas a API ainda não tem adaptador PostgreSQL: hoje a fonte de verdade fiscal em execução é o Firestore.
