...
interface SyncApi{@POST("api/v1/auth/sessions") suspend fun session(@Header("Authorization") bearer:String,@Body payload:Map<String,String>):SessionResponse;@POST("api/v1/sales/offline-sync/orders") suspend fun order(@Header("Authorization") bearer:String,@Header("X-Device-Session") session:String,@Body payload:Map<String,@JvmSuppressWildcards Any>):SyncResponse}
