...
describe('computeMarginPercent', () => {
  it('calcula a margem sobre o preco de venda', async () => {
    expect(computeMarginPercent(100, 200)).toBe(50);
    expect(computeMarginPercent(0, 100)).toBe(100);
  });

  it('nao divide por zero quando o preco de venda e zero', async () => {
    expect(computeMarginPercent(10, 0)).toBe(0);
  });
});

describe('ProductService — cadastro', () => {
  it('cria um produto com todos os campos da secao 5', async () => {
    const service = new ProductService(
      new ProductRepository(new FakeFirestore() as unknown as Firestore),
    );
    const product = await service.create(tenant, baseInput('RC-20KG'));
    expect(product.fiscal.ncm).toBe('23099090');
    expect(product.pricing.salePrice).toBe(189.9);
    expect(product.status).toBe('active');
  });

  it('recusa SKU duplicado no mesmo tenant', async () => {
    const service = new ProductService(
      new ProductRepository(new FakeFirestore() as unknown as Firestore),
    );
    await service.create(tenant, baseInput('RC-20KG'));
    await expect(service.create(tenant, baseInput('RC-20KG'))).rejects.toThrow();
  });
...
describe('ProductService — efeito do status na venda (c8-4)', () => {
  ...
  it.each(cases)('status %s permite venda: %s', async (status, allowed) => {
    expect(PRODUCT_STATUS_ALLOWS_SALE[status]).toBe(allowed);
    const product = await service.create(tenant, { ...baseInput(`SKU-${status}`), status });
    if (allowed) {
      await expect(service.assertCanSell(tenant, product.id)).resolves.toBeDefined();
    } else {
      await expect(service.assertCanSell(tenant, product.id)).rejects.toThrow();
    }
  });
});

describe('ProductService — busca e paginacao por cursor', () => {
  it('pagina os resultados e devolve hasMore ate acabar', async () => {
    const service = new ProductService(
      new ProductRepository(new FakeFirestore() as unknown as Firestore),
    );
    for (let i = 0; i < 5; i += 1) await service.create(tenant, baseInput(`SKU-${i}`));

    const page1 = await service.search(tenant, {}, 2);
    expect(page1.items).toHaveLength(2);
    expect(page1.hasMore).toBe(true);

    const page2 = await service.search(tenant, {}, 2, page1.nextCursor ?? undefined);
    expect(page2.items).toHaveLength(2);
    expect(page2.items[0]?.sku).not.toBe(page1.items[0]?.sku);

    const page3 = await service.search(tenant, {}, 2, page2.nextCursor ?? undefined);
    expect(page3.items).toHaveLength(1);
    expect(page3.hasMore).toBe(false);
  });

  it('busca por SKU, codigo de barras e descricao (c8-6)', async () => {
    const service = new ProductService(
      new ProductRepository(new FakeFirestore() as unknown as Firestore),
    );
    await service.create(tenant, { ...baseInput('RC-20KG'), ean: '4006381333931' });
    const bySku = await service.search(tenant, { q: 'RC-20KG' }, 10);
    const byEan = await service.search(tenant, { q: '4006381333931' }, 10);
    const byName = await service.search(tenant, { q: 'racao premium' }, 10);
...
describe('ProductService — importacao em massa (c8-5)', () => {
  it('importa linhas validas e reporta as invalidas sem parar a planilha', async () => {
    ...
    const result = await service.importCsv(tenant, csv);
    expect(result.imported).toBe(2);
    expect(result.failed).toBe(1);
    expect(result.rows[1]?.ok).toBe(false);
    expect((await service.search(tenant, {}, 10)).items).toHaveLength(2);
  });
});
